...
import { buildHeroHtml, assembleAndWrite } from './pipeline/assemble.js';
...
export async function compile(options: Options): Promise<CompileResult> {
  ...
  return assembleAndWrite(
    options, meta, title, accent, bodyHtml, heroHtml, assetsDir, warnings,
  );
}
